...
class MfccSpectrogram:
    '''
    Stores the common parameters and computes an mfcc spectrogram.
    '''
    def __init__(self, params):
        ...
        self.mfcc_number = params.mfcc_number  # different param
    ...
    def get_spectrogram(self, path_to_audio):
        ...
        Returns:
            - transposed mfcc spectrogram, numpy.ndarray of shape
            (len(wavedata)//self.hop_length-1, self.mfcc_number)
        '''
        ...
        return S# librosa.power_to_db(S, ref=np.max).T


def extract_features(data_type):
    '''
    Extracts features from audio files using mfcc
    Parameters:
    ...
    HDF5_PATH = os.path.join(params.storage_dir, params.features_dir, 'mfcc',
            '{}.h5'.format(data_type))
    ...
    h5_fout.create_dataset(name='feature', shape=(0, mfcc.mfcc_number), dtype=np.float32,
            maxshape=(None, mfcc.mfcc_number))
    ...
    for k, filename in enumerate(metadata_df['fname']):
        ...
        feature = mfcc.get_spectrogram(path_to_audio)

        begin_ind = h5_fout['feature'].shape[0]
        end_ind = begin_ind + feature.shape[0]

        h5_fout['feature'].resize((end_ind, mfcc.mfcc_number))
